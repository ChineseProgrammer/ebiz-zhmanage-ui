<template>
  <div id="business-list" class="only-content">
    ...
    <div class="mb15">
      <Button
        v-if="isAdminCenter || button.agentBusinessList_export"
        @click="exportPolicy"
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          align: "center",
          btn: { check: function () {
              if(self.isAdminCenter || self.button.agentDetail){
                return true;
              }
              return false;
            } },
  ...
  methods: {
    ...
    fetchData(page = 1, pageSize = 10) {
      ...
      this.$api.getAgentPolicyPage(reqData).then((res) => {
        // loading关闭
    ...
    //查询险种列表
    getLiabilityList() {
      this.$api.getAgentLiabilityList({}).then((res) => {
        if (res.result == "0") {
    ...
    //查询机构列表
    getOrgList() {
      this.$api.getAgentOrgList({}).then((res) => {
        if (res.respCode == "0") {
    ...
    exportPolicy() {
      ...
      obj.isCustomer = "1";
      this.$api.batchAgentExportPolicy(obj).then((res) => {
        this.$util.hideLoading();
