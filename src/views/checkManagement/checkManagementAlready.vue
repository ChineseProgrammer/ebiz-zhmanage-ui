<template>
  <div id="business-list" class="only-content">
    <!-- 顶部筛选 -->
    <div>
      <Form ref="formInline" :rules="rules" :model="searchBusiness" inline :label-width="100">
  ...
  <FormItem prop='applyCode' label="投保单号">
            ...
            <FormItem prop="source" label="管理机构名称">
              <Select
                filterable
                clearable
            ...
            <FormItem prop="source" label="是否自主投保">
              ...
              <FormItem prop='orderNo' label="投保人证件号码">
              <Input
             ...
             <!-- <FormItem prop="source" label="提交状态">
              <Select
                filterable
                clearable
              v-model.trim="searchBusiness.result"
                style="width:160px"
                placeholder="请选择"
              >
                <Option
             ...
             <FormItem prop="source" label="审批状态">
              <Select
                filterable
                clearable
              v-model.trim="searchBusiness.status"
                style="width:160px"
                placeholder="请选择"
              >
                <Option
            ...
            <FormItem label="审批时间" >
               <DatePicker
                style="width:160px"
                      :clearable="false"
                        v-model="searchBusiness.approvalTime"
                        type="daterange"
                      ></DatePicker>
            </FormItem>
          <!-- </i-col>
    ...
    <!-- 列表 -->
      <Button v-if="isAdminCenter || button.checkManagementAlreadyExport" type="primary" style="margin-bottom: 10px" @click="exportAll"
        >导出</Button>
...
export default {
     ...
     columns: [
       {
        ...
        },
         {
          ...
          render: (h, params) => {
            ...
            if (params.row.isAutonomy == "1") {
              name = "是";
            } else if (params.row.isAutonomy === "0"||params.row.isAutonomy === 0) {
              name = "否";
            }
            return h("div", name);
          }, 
        ...
        },
        {
          ...
          minWidth: 90,
           render: (h, params) => {
            if (params.row.auditTime) {
              return h("div", dateTransfer(params.row.auditTime));
            }
          },
        },
        {
  ...
  exportAll(){
        this.$api
        .exportApprovalTemplate(this.searchBusiness)
        .then((res) => {
          this.$util.hideLoading();
          let elink = document.createElement("a");
          elink.download = "已审批信息.xlsx";
    ...
    //客户查询
    changeFormSearch() {
      if(this.searchBusiness.approvalTime&&this.searchBusiness.approvalTime[0]){
          this.searchBusiness.approvalStartTime =this.searchBusiness.approvalTime[0].getTime();
            this.searchBusiness.approvalEndTime = this.searchBusiness.approvalTime[1].getTime();
      }
    ...
    fetchData(page = 1, pageSize = 10) {
      ...
      this.$api.approvalPage(reqData).then((res) => {
        // loading关闭
        this.$util.hideLoading();
        // 成功判断
        if (res.result === "0") {
          ...
          this.tableList = res.content.list.map(el=>{
            el.chargePeriod=['无关',
                   ...
                   '不定期交',
                   '按月交'][el.chargePeriod]
            el.coveragePeriod=['无关',
                    '保终身',
                    '按年限保',
                    '保至某确定年龄',
                    '按月保',
                    '按天保'][el.coveragePeriod]
            return el
          });
          //分页数据
    ...
    businessCheck(val) {
      this.$router.push({
        ...
        params: { policyId: val.approvalNo ,type:'already'},
      });
    },
    exportPolicy() {
      this.$util.showLoading();
