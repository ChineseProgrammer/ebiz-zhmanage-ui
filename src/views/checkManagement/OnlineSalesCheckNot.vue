<template>
  <div id="business-list" class="only-content">
    ...
    <!-- 列表 -->
    <Button
      v-if="isAdminCenter || button.checkOnlineSalesExport" type="primary" style="margin-bottom: 10px"  @click="exportAll"
>导出</Button
    >
    <div>
      <primaryTable
...
export default {
  ...
  methods: {
    exportAll() {
      this.$api
        .checkOnlineSalesExport(this.searchBusiness)
        .then((res) => {
    ...
    fetchData(page = 1, pageSize = 10) {
      ...
      this.$api.notApprovalOnlineSalesPage(reqData).then((res) => {
        // loading关闭
        this.$util.hideLoading();
        // 成功判断
        if (res.result === "0") {
