<template>
  <div id="set-list" class="only-content header">
    ...
    >
      <Row>
        <Col span="6">
          <FormItem prop="nameOrCode" label="活动编码">
            <Input
              placeholder="请输入活动编码"
              v-model.trim="paramsGroup.activityId"
              style="width: 200px"
            />
          </FormItem>
        </Col>
        <Col span="6">
          <FormItem prop="customerId" label="留资活动机构">
            <Select
              clearable
              filterable
              style="width: 200px"
              placeholder="请选择"
              v-model="paramsGroup.organizationName"
            >
              <Option
        ...
        <Col span="6">
          <FormItem style="width: 100%" label="注册日期">
            <DatePicker
            transfer
              style="width: 200px"
              type="daterange"
           ...
           <Col span="6">
          <FormItem style="width: 100%" label="留资日期">
            <DatePicker
            transfer
              style="width: 200px"
              type="daterange"
         ...
         <Col span="6">
          <FormItem prop="activityGroup" label="留资类型">
            <Select
              clearable
              filterable
              style="width: 200px"
              placeholder="请选择"
              v-model="paramsGroup.activityGroup"
            >
              <Option
                value="收货留资"
                >收货留资</Option
    ...
    <Button  v-if="isAdminCenter || button.retention_listExport" type="primary" @click="exportAll">批量导出</Button>
    <primaryTable
      :columns="deptColumns"
      :data="deptTableList"
      ref="primaryTable"
...
export default {
  ...
  data() {
    return {
        ...
        },
        {
          ...
          render: (h, params) => {
            if (params.row.registerTime) {
              return h("div", dateTransfer(params.row.registerTime));
            }
          },
        },
        {
        ...
        },
         {
          ...
          render: (h, params) => {
            if (params.row.createTime) {
              return h("div", dateTransfer(params.row.createTime));
            }
          },
        },
           {
      ...
      formRuleGroup: {
        activityId: [this.$util.rules.required()],
  ...
  methods: {
    ...
    exportAll() {
      this.$api
        .retainedcapitalExport(this.paramsGroup)
        .then((res) => {
          this.$util.hideLoading();
          let elink = document.createElement("a");
          elink.download = "留资管理信息.xlsx";
    ...
    onDatePickerChange(val) {
      ...
      if (val[0]) {
        this.paramsGroup.createTimeStart = val[0]+" 00:00:00";
        this.paramsGroup.createTimeEnd = val[1]+" 23:59:59";
      } else {
        this.paramsGroup.createTimeStart = null;
        this.paramsGroup.createTimeEnd = null;
      }
    },
    getManageOrgList() {
    ...
    search(page) {
      ...
      this.$api
        .retainedcapital(_self.paramsGroup)
        .then((res) => {
    ...
    reset() {
      ...
      this.paramsGroup.activityGroup=""
