/*
 * @Date: 2020-08-11 16:21:26
 * @LastEditors: 周靖松
 * @LastEditTime: 2020-08-19 17:25:57
 */
...
var rules={
            //必填项
            required:(message,trigger)=>{ return  { required: true, message: message?message:'必填项不能为空', trigger: trigger?trigger:'blur,change' }},
            //邮件
            ...
            validateAge:(message,trigger)=>{
             ...
             words:(message,trigger)=>{ return {pattern:"^[a-zA-Z\u4e00-\u9fa5]{1,50}$", message: message?message:"1-50位中英文字符", trigger: trigger?trigger:'blur,change'}},

             //字母数字50位
             numberCharMax50:(message,trigger)=>{ return { pattern:"^[a-zA-Z0-9]{1,50}$", message: message?message:'输入值为1-50位数字、字母！',trigger:trigger?trigger:'blur'}},
             //字母数字50位
             numberMax50:(message,trigger)=>{ return { pattern:"^[0-9]{1,50}$", message: message?message:'输入值为1-50位纯数字！',trigger:trigger?trigger:'blur'}},
